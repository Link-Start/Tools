...
- (void)setupLayout
{
    ...
    //下面的代码，都不能注释(否则cell不显示)
    self.headerReferenceSize = CGSizeMake(CGRectGetWidth(self.collectionView.frame), 30);//设置section header 固定高度，如果需要的话
    self.footerReferenceSize = CGSizeMake(CGRectGetWidth(self.collectionView.frame), CGFLOAT_MIN);//设置section footer 固定高度，如果需要的话
    self.sectionInset = UIEdgeInsetsMake(10, 5, 5, 0);//设置
...
//        NSArray *sectionYArray = [_framesArray yl_objectAtIndex:section];
        NSArray *sectionYArray = [NSArray array];//***************************************
