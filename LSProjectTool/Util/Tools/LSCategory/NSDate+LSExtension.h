...
+ (NSDate *_Nullable)convertDateToLocalTime:(NSDate *_Nonnull)forDate;

#pragma mark - 相对日期
/** 现在的日期  @return 现在的日期 */
+ (NSDate *_Nonnull)dateNow;
/** 明天的日期  @return 明天的日期 */
...
- (NSString *_Nullable)stringWithDateStyle:(NSDateFormatterStyle)dateStyle timeStyle:(NSDateFormatterStyle)timeStyle;
/*** 通过format格式将当前日期转换为String格式 @param format 格式样式  @return 转换后得到的String */
- (NSString *_Nonnull)stringWithFormat:(NSString *_Nonnull)format;
...
- (NSInteger)distanceInDaysToDate:(NSDate *_Nonnull)anotherDate;
...
- (NSDate *_Nonnull)previousMonthDate;

///获得当前 NSDate 对象的下个月的某一天（此处定为15号）的 NSDate 对象
- (NSDate *_Nonnull)nextMonthDate;

///获得当前 NSDate 对象对应月份根据所传入的day所属日期
- (NSDate *_Nonnull)otherDayInMonth:(NSInteger)number;
