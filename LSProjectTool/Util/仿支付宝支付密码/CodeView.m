...
@property(nonatomic,assign) UIFont *labelFont;//验证码字体大小
...
- (void)setUpSubview {
    //每一个验证码的宽度
    CGFloat width = (CGRectGetWidth(self.bounds) - (self.codeNumber-1)*self.space)/self.codeNumber;
    [self addSubview:self.textField];
    self.textField.frame = self.bounds;
    
    for (int i = 0; i < self.codeNumber; i ++) {
        UIView *subView = [[UIView alloc] init];
        subView.frame = CGRectMake((width+self.space)*i, 0, width, self.height);
        ...
        if (self.style == CodeStyle_line) {
            layer.frame = CGRectMake(4, self.height-1, width-8, 1);
            if (i == 0) {
            ...
            width = self.height;
            CGFloat temSpace = 0.0;//间隔
            temSpace = (self.width - self.codeNumber*self.height)/((self.codeNumber-1)*1.0);
            subView.frame = CGRectMake((width+temSpace)*i, 0, width, self.height);
        ...
        label.frame = CGRectMake(0, 0, width, self.height);
        ...
        //光标
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(width/2, (self.height/2)-8, 1,16)];
