...
- (void)addLA {
    ...
    [self.widthArray addObject:[NSNumber numberWithFloat:self.lineWidth]];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.pointArray.count > 0) {
        ...
        for (int i = 1; i < [self.pointArray count]; i++) {
            CGPoint myEndPoint = CGPointFromString([self.pointArray objectAtIndex:i]);
            CGContextAddLineToPoint(context, myEndPoint.x, myEndPoint.y);
        }
...
- (void)saveImage:(void(^)(UIImage *signature))saveSuccessBlock {
    if (self.lineArray.count > 0) {
        ...
        self.signatureImage = [UIImage imageWithCGImage:newImageRef];
        ...
        if (saveSuccessBlock) {
            saveSuccessBlock(self.signatureImage);
        }
    } else {
