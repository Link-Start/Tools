...
static const CGFloat kTOSearchBarBackgroundHeightModern = 30.0f;
...
- (void)setUpViews
{
    ...
    self.alignment = TOSearchBarAlignmentLeft;//靠左
}
...
- (void)layoutSubviews
{
    ...
    frame.size.height = self.isSelfHeight?self.frame.size.height:kTOSearchBarBackgroundHeightClassic;
    
    if (@available(iOS 11.0, *)) {
        frame.size.height = self.isSelfHeight?self.frame.size.height:kTOSearchBarBackgroundHeightModern;
    }
    if (self.editing && self.cancelButton) {
...
//    CGFloat placeholderLabelWidth = CGRectGetWidth(frame);
//    CGFloat iconViewWidth = self.iconView.frame.size.width;
//
//    if (self.alignment == TOSearchBarAlignmentLeft) {//靠左
//        frame.origin.x = (kTOSearchBarInset) + self.iconView.frame.size.width;
//        frame.origin.x += iconMargin;
//    } else if (self.alignment == TOSearchBarAlignmentTextCenter) {//文字居中
//        frame.origin.x = floorf((containerViewWidth - placeholderLabelWidth) * 0.5f);
//    } else if (self.alignment == TOSearchBarAlignmentIconAndTextCenter) {//icon和文字 居中
//        frame.origin.x = floorf((containerViewWidth-placeholderLabelWidth-iconMargin-iconViewWidth)*0.5);
//    } else if (self.alignment == TOSearchBarAlignmentRight) {//靠右
//        frame.origin.x = floorf(CGRectGetWidth(self.containerView.frame) - CGRectGetWidth(frame));
//    }
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    void (^selectedBlock)(void) = ^{
        ...
        // 点击搜索框，使搜索框成为第一响应者时 颜色有一个闪烁变化，就是这里
...
- (void)textFieldDidChange:(UITextField *)textField
{
    ...
    if (!textrange) {
        if ([self.delegate respondsToSelector:@selector(searchBar:textAssociateEnd:)]) {
            [self.delegate searchBar:self textAssociateEnd:self.text];
        }
    }
}
...
- (void)setAlignment:(TOSearchBarAlignment)alignment {
    _alignment = alignment;
}

- (void)setLeftImage:(UIImage *)leftImage {
    _leftImage = leftImage;
    self.iconView.image = leftImage;
}
