...
/**
 The YYLabel class implements a read-only text view.YYLabel类实现了只读文本视图
 
 @discussion The API and behavior is similar to UILabel, but provides more features:API的功能类似于UILabel，但提供了更多功能

 * It supports asynchronous layout and rendering (to avoid blocking UI thread).它支持异步布局和渲染（以避免阻塞UI线程）
 * It extends the CoreText attributes to support more text effects.它扩展了CoreText属性以支持更多的文本效果
 * It allows to add UIImage, UIView and CALayer as text attachments.它允许添加UIImage、UIView和CALayer作为文本附件
 * It allows to add 'highlight' link to some range of text to allow user interact with.它允许在一定范围的文本中添加“突出显示”链接，以允许用户与之交互
 * It allows to add container path and exclusion paths to control text container's shape.它允许添加容器路径和排除路径来控制文本容器的形状
 * It supports vertical form layout to display CJK text.它支持显示CJK文本的垂直形式布局
...
#pragma mark - Accessing the Text Attributes 访问文本属性
///=============================================================================
/// @name Accessing the Text Attributes 访问文本属性
///=============================================================================
...
/**
 ...
 标签显示的文本.默认值为nil
 为此属性设置新值也会替换“attributedText”中的文本
 返回`attributedText`中的纯文本
 */
...
/**
 ...
 文本的字体。默认为17-point 系统字体
 为此属性设置一个新值也会使新字体应用于整个“attributedText”
 返回“attributedText”开头的字体
 */
...
/**
 ...
 文本的颜色。默认值为黑色。
 为此属性设置一个新值也会使新颜色应用于整个“attributedText”。
 Get the value返回“attributedText”开头的颜色。
 */
...
/**
 ...
 文本的阴影颜色。默认值为nil。
 为此属性设置新值还会导致阴影颜色应用于整个“attributedText”。
 Get the value返回“attributedText”开头的阴影颜色。
 */
...
/**
 ...
 文本的阴影偏移量。默认值为CGSizeZero。
 为此属性设置一个新值还会导致阴影偏移应用于整个“attributedText”。
 Get the value返回“attributedText”开头的阴影偏移量。
 */
...
/**
 ...
 文本的阴影模糊。默认值为0。
 为此属性设置新值还会导致阴影模糊应用于整个“attributedText”。
 Get the value返回“attributedText”开头的阴影模糊。
 */
...
/**
 ...
 用于对齐文本的技术。默认值为NSTextAlignmentNatural。
 为此属性设置一个新值也会使新对齐方式应用于整个“attributedText”。
 Get the value返回“attributedText”开头的对齐方式。
 */
@property (nonatomic) NSTextAlignment textAlignment;

/**
 The text vertical aligmnent in container. Default is YYTextVerticalAlignmentCenter.
 容器中的文本垂直对齐。默认值为YYYTextVerticalAlignmentCenter。
 */
