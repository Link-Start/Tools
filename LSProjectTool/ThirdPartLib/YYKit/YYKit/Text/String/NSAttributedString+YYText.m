...
/// 设置间距，附件 文本空格宽度， 文本左侧/右侧 空格占位宽度
+ (NSMutableAttributedString *)attachTextSpaceWidth:(CGFloat)width {
    NSMutableAttributedString *attachTextSpace = [NSMutableAttributedString attachmentStringWithContent:[UIView new] contentMode:UIViewContentModeCenter width:width ascent:0 descent:0];
...
/// 添加图片到富文本
+ (NSMutableAttributedString *)attachmentImageWithImageName:(nullable NSString *)imageName
                                               scale:(CGFloat)scale
                                            attachmentSize:(CGSize)attachmentSize
                                               alignToFont:(UIFont *)font
                                                  alignment:(YYTextVerticalAlignment)alignment {
    ...
    return [self attachmentImageWithImage:image scale:scale attachmentSize:attachmentSize alignToFont:font alignment:alignment];
}
+ (NSMutableAttributedString *)attachmentImageWithImage:(nullable UIImage *)image
                                               scale:(CGFloat)scale
                                            attachmentSize:(CGSize)attachmentSize
                                               alignToFont:(UIFont *)font
                                              alignment:(YYTextVerticalAlignment)alignment {
    ...
    //如果 attachmentSize 为 CGRectZero
    if (CGSizeEqualToSize(attachmentSize, CGSizeZero)) {
        attachmentSize = image.size;
    }
    
    NSMutableAttributedString *attachImage = [NSMutableAttributedString attachmentStringWithContent:image contentMode:UIViewContentModeCenter attachmentSize:attachmentSize alignToFont:font alignment:YYTextVerticalAlignmentCenter];
