 ...
 */
/**
 YYDiskCache是一个线程安全缓存，用于存储SQLite支持的键值对和文件系统（类似于NSURLCache的磁盘缓存）.
 YYDiskCache具有以下功能：
 * 它使用LRU（最近最少使用）来移除对象
 * 它可以通过成本、数量和年龄来控制
 * 它可以配置为在没有可用磁盘空间时自动收回对象
 * 它可以自动决定要获取的每个对象的存储类型（sqlite/file）更好的性能
 你可以编译sqlite的最新版本,并忽略libsqlite3.dylib, iOS系统的速度可以提高2~4倍
 */
@interface YYDiskCache : NSObject

#pragma mark - 属性 Attribute
///=============================================================================
/// @name 属性 Attribute
///=============================================================================
 ...
 */
/// 如果对象的数据大小（以字节为单位）大于此值，则对象将存储为文件，否则对象将存储在sqlite中.
/// 0表示所有对象都将存储为单独的文件，NSUIntegerMax表示所有对象将存储在sqlite中.
/// 默认值为20480（20KB）
 ...
 */
/// 如果这个block不是nil，则这个block将用于归档对象，而不是NSKeyedArchiver。您可以使用此block来支持不符合“NSCoding”协议的对象
/// 默认值为nil
...
/**
 ...
 The default value is nil.
 */
/// 如果这个block不是nil，则该block将用于取消归档(解档)对象,而不是NSKeyedUnarchiver。您可以使用此block来支持不符合“NSCoding”协议的对象。
/// 默认为nil
...
/**
 ...
 The default value is nil.
 */
/// 当对象需要保存为文件时，将调用此block为指定的键生成文件名。如果block为nil，则缓存使用md5(键)作为默认文件名。
