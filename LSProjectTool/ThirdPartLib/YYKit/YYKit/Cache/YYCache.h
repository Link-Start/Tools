 ...
 */
/// YYCache是线程安全的键值缓存
/// 它使用“YYMemoryCache”将对象存储在小型快速内存缓存中,并使用“YYDiskCache”将目标持久化到大型慢速磁盘缓存中。
/// 详细信息请参阅“YYMemoryCache”和“YYDiskCache”
 ...
 */
/// 用 block 清空缓存。此方法立即返回，并在后台执行 block 清除操作。
/// @warning 你不应该在这些 block 中将消息发送到这个实例
/// @param progress 删除期间将调用这个block，传递 nil 以忽略。
/// @param end 此 block 将在末尾调用，传递 nil 以忽略。
