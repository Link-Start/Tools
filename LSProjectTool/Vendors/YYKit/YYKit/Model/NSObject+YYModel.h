...
/**
 Provide some data-model method: 提供一些数据模型方法
 
 * Convert json to any object, or convert any object to json.将json转换为任何对象，或将任何对象转换为json
 * Set object properties with a key-value dictionary (like KVC).使用键值字典(如KVC) 设置对象属性
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`. `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`的实现
 
 See `YYModel` protocol for custom methods. 有关自定义方法，请参见 "YYModel" 协议
 
 
 Sample Code:示例代码：
    
     ********************** json convertor josn转换器*********************
 ...
 */
/// 从 json 创建并返回接收器的新实例。这种方法是线程安全的
/// @param json 是`NSDictionary`, `NSString` or `NSData` 中的 json 对象
/// @return 返回一个从 json 创建的新实例，如果出现错误，返回nil
...
/// 从键值字典创建并返回接收器的新实例。这种方法是线程安全的。
/// @param dictionary 映射到实例属性的键值字典。字典中的任何无效键值对都将被忽略。
/// @return 返回从字典创建的新实例，如果发生错误，返回nil。
/// @discussion ‘dictionary’ 中的键将映射到接收者的属性名，该值将设置为属性。如果值的类型与属性不匹配，此方法将尝试根据以下规则转换值：
///           `NSString` or `NSNumber` -> c 数字, 比如 BOOL, int, long, float, NSUInteger...
///           `NSString` -> NSDate, 用 "yyyy-MM-dd'T'HH:mm:ssZ", "yyyy-MM-dd HH:mm:ss" or  "yyyy-MM-dd". 格式解析
///           `NSString` -> NSURL.
///           `NSValue` -> struct 或 union, 例如 CGRect, CGSize, ...
///           `NSString` -> SEL, Class."""
