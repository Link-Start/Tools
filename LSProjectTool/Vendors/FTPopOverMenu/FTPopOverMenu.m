...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.optionsListLimitHeight = 0; //最大高度
    }
    return self;
}
...
- (void)adjustPopOverMenu {
    ...
    if (self.sender) {
        ...
        // if run into touch problems on nav bar, use the fowllowing line.如果在导航栏上遇到接触问题,请使用下面一行
...
#pragma mark - 修改这里，增加设置最大高度判断
    // 如果配置了menu的最大高度 && menu的高度超过了配置的高度
    if (self.config.optionsListLimitHeight != 0 && menuHeight > self.config.optionsListLimitHeight) {
    ...
    if (arrowDirection == FTPopOverMenuArrowDirectionUp) {//箭头方向向上
